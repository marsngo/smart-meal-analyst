...
# ==================== معالجة وعرض البيانات الموجهة ====================
kobo_data = st.session_state.kobo_data

if kobo_data is not None:
    st.markdown("---")
    # 🎯 البوابة الرئيسية لنوع التحليل (الموجه للهدف)
    analysis_goal = st.selectbox(
        "🎯 اختر نوع ومسار التحليل الذي ترغب بتشغيله الآن على الملف:",
        (
            "1. لوحة البيانات التنفيذية والتدقيق الآلي لجودة البيانات (Dashboard & QA)",
            "2. مقارنة وتتبع التقييم القبلي والبعدي وقياس الأثر (Pre vs Post Assessment)",
            "3. تحليل مشاعر الردود النصية والشكاوى والفرز الجغرافي (AI Sentiment Analysis)",
            "4. مصفوفة التحليل والمقارنات العامة المتقاطعة (Bulk Cross Analysis)"
        )
    )
    st.markdown("---")

    # ==================== المسار الأول: لوحة البيانات والتدقيق ====================
    if "1. لوحة البيانات" in analysis_goal:
    ...
    # ==================== المسار الثاني: القبلي والبعدي ====================
    elif "2. مقارنة وتتبع" in analysis_goal:
            ...
            if score_column != "لا يوجد" and type_column != "لا يوجد" and user_id_column != "لا يوجد":
                ...
                if not pre_rows.empty and not post_rows.empty:
                    ...
                    if not paired_df.empty:
                        ...
                        st.warning("⚠️ فشل الربط التلقائي. تأكد من أن أسماء المستفيدين مكتوبة بنفس الأحرف والاملاء تماماً في المقابلتين.")
                else:
                    st.warning("⚠️ لم نجد الكلمات الدلالية ('قبلي' أو 'بعدي') داخل العمود المختار.")
        ...
        if calculation_ready and not pd.isna(mean_pre) and not pd.isna(mean_post):
            ...
            st.error("⚠️ فشل حساب الأرقام. يرجى التحقق من أن عمود الدرجة يحتوي على أرقام صافية.")

    # ==================== المسار الثالث: تحليل المشاعر ====================
    elif "3. تحليل مشاعر" in analysis_goal:
        st.header("🧠 تحليل مشاعر الردود المفتوحة والربط الجغرافي (AI Sentiment Analysis)")
        
        if not api_key:
            st.info("💡 لتشغيل ميزة تحليل المشاعر، يرجى تفعيل مفتاح Gemini API في الشريط الجانبي أولاً.")
        else:
            ...
            if text_column != "لا يوجد" and geo_name_column != "لا يوجد":
                valid_rows = kobo_data[[text_column, geo_name_column]].dropna().head(40)
                
                if not valid_rows.empty:
                    if st.button("🚀 تشغيل محرك تحليل المشاعر والربط الجغرافي"):
                        with st.spinner("جاري استدعاء الـ AI وتحليل ردود المستفيدين..."):
                            try:
                                data_payload = []
                                ...
                                ⚠️ شروط صارمة: اكتب الجدول مباشرة دون أي مقدمات لكي نقوم بقراءته برمجياً بنجاح."""
                                ...
                                if clean_rows:
                                    ...
                                    # --- حساب النسب المئوية للمشاعر وتوليد الشارتات الدائرية الفخمة ---
                                    st.markdown("### 📊 الملخص الإحصائي والنسب المئوية للمشاعر (Sentiment Percentages)")
                                    
                                    # حساب تكرار النسب
                                    sentiment_counts = sentiment_df["التصنيف"].value_counts().reset_index()
                                    sentiment_counts.columns = ["التصنيف", "العدد"]
                                    sentiment_counts["النسبة (%)"] = ((sentiment_counts["العدد"] / sentiment_counts["العدد"].sum()) * 100).round(1)
                                    
                                    chart_pct_col, chart_geo_col = st.columns(2)
                                    with chart_pct_col:
                                        # شارت دونات تفاعلي للنسب المئوية الإجمالية
                                        fig_donut = px.pie(sentiment_counts, values="العدد", names="التصنيف", hole=0.5,
                                                           title="النسبة المئوية الإجمالية لرضا ومشاعر المستفيدين",
                                                           color="التصنيف",
                                                           color_discrete_map={"إيجابي": "#10b981", "سلبي": "#ef4444", "محايد": "#f59e0b"})
                                        st.plotly_chart(fig_donut, use_container_width=True)
                                        
                                    with chart_geo_col:
                                        # شارت الأعمدة الجغرافي
                                        fig_sent = px.histogram(sentiment_df, x="المنطقة", color="التصنيف", barmode="group",
                                                                 title="توزيع رضا المستفيدين حسب المناطق الجغرافية الاسمية",
                                                                 color_discrete_map={"إيجابي": "#10b981", "سلبي": "#ef4444", "محايد": "#f59e0b"})
                                        st.plotly_chart(fig_sent, use_container_width=True)
                                    
                                    # عرض جدول البيانات والنسب الصريحة
                                    tbl_col1, tbl_col2 = st.columns([1, 2])
                                    with tbl_col1:
                                        st.markdown("🔢 **جدول النسب المئوية للمشاعر:**")
                                        st.dataframe(sentiment_counts, use_container_width=True)
                                    with tbl_col2:
                                        st.markdown("📋 **الجدول التفصيلي المفصل للردود:**")
                                        st.dataframe(sentiment_df, use_container_width=True)
                                else:
                                    st.error("فشل معالجة الرد برمجياً. يرجى إعادة المحاولة.")
                            except Exception as e:
                                st.error(f"حدث خطأ أثناء الاتصال بالذكاء الاصطناعي: {e}")
                else:
                    st.warning("⚠️ لا توجد ردود نصية صالحة للتحليل.")

    # ==================== المسار الرابع: المقارنات العامة والتقارير ====================
    else:
        st.header("🎛️ مصفوفة التحليل والمقارنات العامة المتقاطعة (Bulk Cross Analysis)")
        ...
        if main_questions:
            ...
            if cross_questions:
                ...
                if not api_key:
                    st.info("💡 لتوليد تقارير سردية، يرجى تفعيل الـ API Key أولاً.")
                else:
